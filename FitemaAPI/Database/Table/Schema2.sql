...
CREATE TABLE IF NOT EXISTS `bills` (
  ...
  PRIMARY KEY (`Id`),
  KEY `FK_Bilss_Status` (`StatusId`) USING BTREE,
  KEY `FK_Bills_Organizations` (`OrgId`),
  KEY `FK_Bills_Plans` (`PlanId`),
  CONSTRAINT `FK_Bills_Organizations` FOREIGN KEY (`OrgId`) REFERENCES `organizations` (`Id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_Bills_Plans` FOREIGN KEY (`PlanId`) REFERENCES `plans` (`Id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_Bills_Status` FOREIGN KEY (`StatusId`) REFERENCES `status` (`Id`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE IF NOT EXISTS `customers` (
  ...
  `UpdatedAt` datetime DEFAULT NULL,
  PRIMARY KEY (`Id`),
  KEY `FK_Customers_Organization` (`OrgId`),
  CONSTRAINT `FK_Customers_Organization` FOREIGN KEY (`OrgId`) REFERENCES `organizations` (`Id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=armscii8 COLLATE=armscii8_bin;
