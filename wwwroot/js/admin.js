function cell_prepared(e) {
    if (e.rowType === "data" && e.column.dataField === "status") {
        ...
        e.cellElement.css("color", e.data.status == "Accepted" ? "#0BB972" : e.data.status == "Rejected" ? "#A70E0E" : "#D4A400");
    }
}

function nullDateCustom(e) {
    if (e.valueText == "01 January 1901") {
        return "-";
    }
    return e.valueText;
}

function currencyCustom(e) {
    return e.valueText.replace("$", "RM ");
}

function ExportNow(e, name) {
    var itemExported = [];
    ...
    DevExpress.excelExporter.exportDataGrid({
        ...
        customizeCell: function (options) {
            ...
            if (gridCell.data) {
                var data = gridCell.data;
                if (gridCell.column.dataField === 'isDrawn') {
                    console.log("is drawn");
                    excelCell.value = true;
                    addItemExported(itemExported, data.id);
                }
            }
        ...
        if (itemExported.length > 0) {
            drawnTransaction(itemExported);
        }
...
function addItemExported(itemExported, item) {
    if (itemExported.indexOf(item) === -1) {
        itemExported.push(item);
    }
}

function drawnTransaction(itemExported) {
    console.log("drawn");
    var data = {
        transactionsId: itemExported
    };

    $.ajax({
        url: "/Transaction/CreateBatchTransactionDrawn",
...
(function (global, factory) {
    ...
    var PushMenu = /*#__PURE__*/function () {
        ...
        _proto.collapse = function collapse() {
            var $bodySelector = $__default['default'](SELECTOR_BODY);
            $("#btn_switch_menu").toggleClass("fa-bars fa-times");
            if (this._options.autoCollapseSize && $__default['default'](window).width() <= this._options.autoCollapseSize) {
